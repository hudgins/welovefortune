...
function screens.start_draw()
 ...
 if (start.display_time + 4.5 < time()) then
  ...
  print("press x / ❎ button", 26, 100, clr_flashing(true))
 end
end
function screens.start_update()
 local start = screens.start_view
 if (start.display_time + 4 > time()) return
...
wheel_items = {
 ...
 split("retreat_9_9900,plant_14_3,vacation_10_800,shed_8_85,bicycle_12_70,scooter_10_180,watch_14_12,ayahuasca_12_50,tuque_13_3,phone_8_120,computer_10_300,tree_12_110,bucket_14_21,tv_12_333,table_10_158,couch_13_423,truck_8_4500,t-shirt_12_1,cat_10_25,dog_14_35,boat_8_2500,hot tub_12_700,car_13_3900,vacuum_10_8"),
}
...
function restart()
 ...
 local function reveal_puzzle()
  toggle_theme_music(0)
  puzzle = to_puzzle("wheel^ of^ retreat^ guru", "")
...
function end_game()
 ...
 if (game_active_player.human) then
  puzzle.revealed = true
  w_bonusboard.lines[3] = "you won the "..w_wheel.item_name.."!"
  w_bonusboard.lines[5] = float_to_money_str(game_active_player.game_total).." grand total"
 end
...
puzzles = split("payments_merchant of record~ payments_paystand paysafe paypal payouts~ centres_temple of the way of light~ centres_expanding light~ payments_medicine not included~ payments_freemium^ to^ stripe~ payments_transactional pricing~ place_the jam factory~ event_open heart circle~ projects_synthesis to^ resurgence~ books_fifteen commitments of conscious leadership~ phrase_i learned it from watching you!~ phrase_what problem are you trying to solve?~ phrase_bump the cache~ phrase_dev down^ dev clean^ dev nuclear^ dev up~ person_change prevention agent~ person_curmudgeon control~ person_senior^ vice president^ of naming~ event_thirsty thursday~ event_margarita monday", "~")
intermission_facts = split("retreat guru got its start\nway back in 1812,\ncrafting surrender flags?~retreat guru was founded by\nfive brothers,\ntwo of which survived?~retreat guru employs\nfour hundred and\nseventy-five thousand people?~retreat guru invented\nthe concept of\ndisintermediation?~retreat guru was the official\nretreat-booking website\nof the 2010 winter olympics?", "~")
