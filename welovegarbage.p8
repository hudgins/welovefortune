...
function is_vowel(letter)
 return includes(c_letters.vowels, letter)
end
function article(letter)
 if (includes(c_letters.article_an, letter)) return "an"
 return "a"
end
...
--      printh("cell:"..cell.letter..","..tostr(cell.garbage)..","..tostr(cell.revealed))
      if (cell.garbage and (cell.letter == letter or (cell.letter and letter == "*"))) count += 1
      if ((not cell.garbage and (cell.letter == letter or (cell.letter and letter == "*"))) and not cell.revealed) count += 1
    end
  end
 end
 return count
...
function add_garbage(puzzle_letters)
 local garbage_puzzle = ""
 for l in all(puzzle_letters) do
   if (l != " ") garbage_puzzle ..= l
 end
 local garbage_letter_options = {}
 for c in all (c_letters.consonants) do
   if (not includes(puzzle_letters, c)) add(garbage_letter_options, c)
 end
 ...
 while (garbage_amount > 0) do
  ...
  garbage_puzzle = garbage_puzzle_temp
  garbage_amount -= 1
 end
 local i = 1
 local with_spaces = ""
 for l in all(garbage_puzzle) do
   if (i == 12) with_spaces ..= " "
   if (i == 12 + 13) with_spaces ..= " "
   if (i == 12 + 13 + 13) with_spaces ..= " "
   with_spaces ..= l
   i += 1
 end
 return with_spaces, garbage_letters
end
