...
game_version = "v1_20231125"

c_clr_theme = 9 -- 10 for normal

c_num_vowel_cost = shr(25, 16)
c_num_grand_prize = shr(2121, 16)
...
function screens.start_draw()
 ...
 if (start.display_time + 4.5 < time()) then
  color(12)
  print(" heavily-inspired by\nthe commodore 64 game", 26, 116)
  print(game_version, 80, 0, 5)
 end
...
function screens.intermission_draw()
 ...
 local x0, y0, x1, y1 = view.x, view.y, view.x + view.w, view.y + view.h
...
function screens.setup_draw()
 ...
 if (#game_players.one.name > 1 and time_remaining > 0 and time_remaining < 3) print("starting game in... "..tostr(time_remaining), x, y + 32, 7)
end
...
function screens.round.state_spin_completed()
 ...
 if (w_wheel.item_name == "loseseed") then
  start_shake(3, 20, {
   w_wheel,
   w_scoreboard,
   w_messageboard,
 ...
 elseif (w_wheel.item_name == "loseturn") then
  ...
  sfx(0)
  w_messageboard_set_message(" "..c_sat_symbol_render.." "..w_wheel.item_name)
...
function bonus_choose_letters()
 ...
 w_bonusboard.lines = split(float_to_money_str(c_num_grand_prize).." grand prize^^choose 3 consonants:^_ _ _^and one vowel:^_", "^")
 local function on_pick()
...
wheel_items = {
 split("loseseed_0,35_14,25_13,60_9,40_8,15_12,25_14,40_9,20_13,loseturn_10,45_8,15_12,20_9,70_14,freespin_11,20_8,30_12,40_13,50_10,100_14,20_9,30_8,80_12,75_10"),
 split("loseseed_0,60_14,20_12,100_10,60_8,30_14,70_9,45_12,15_13,80_8,loseturn_10,50_14,40_12,25_13,loseseed_0,90_9,30_8,25_12,90_10,20_14,40_12,55_13,20_9,50_8"),
 split("loseseed_0,150_10,35_14,90_9,30_8,25_12,90_10,20_14,40_12,55_13,20_9,50_8,loseseed_0,60_14,20_12,loseturn_10,35_13,25_8,50_12,500_7,30_14,80_8,50_12,70_13"),
}
function w_wheel_init(round)
 ...
 for s=1,#w_wheel.items do
  local item = split(w_wheel.items[s], "_")
  w_wheel.item_values[s] = tonum(item[1]) or 0
  w_wheel.item_names[s] = render_money(w_wheel.item_values[s], true)
...
function w_round_draw()
 ...
 if (game_round == 4) print("   bonus", x, y + 2, 12) else print("  rnd: "..game_round, x, y + 2, 12) 
end
...
function restart()
 ...
 local function reveal_puzzle()
  ...
  puzzle = to_puzzle("wheel of bitcoin", "")
...
function end_game()
 ...
 if (game_active_player.human) then
  puzzle.revealed = true
  w_bonusboard.lines[6] = float_to_money_str(game_active_player.game_total).." grand total"
 end
...
function action_solve()
 ...
 local on_all_incorrect_fn, on_all_correct_fn
...
function adjust_spin()
 if (w_wheel.speed > 0.02) then
  ...
  if (w_wheel.cheat and (w_wheel.item_name == "loseseed" or w_wheel.item_name == "loseturn")) then
   w_wheel.speed = 0.5
...
function to_tiles(puz)
 local maxcols, row, rowcol = { 11, 13, 13, 11 }, 1 , 1
...
function render_money(value, no_symbol)
 local padding = (value == "0" or value == 0) and "" or "00000"
 local rendered, converted =
   value..padding
 , ""
 local symbol = no_symbol and "" or c_sat_symbol_render
 if (#rendered < 9) return symbol..rendered
 local left_padding = 8 - #rendered
 if (left_padding >= 0) then
  converted = "0."
  while left_padding > 0 do
   left_padding -= 1
   converted ..= "0"
  end
 elseif (left_padding < 0) then
  converted = sub(rendered, 1, abs(left_padding)).."."..sub(rendered, abs(left_padding) + 1)
  rendered = ""
 end
 rendered = converted..rendered
 converted = ""
 for i=#rendered,1,-1 do
  if (converted != "" or rendered[i] != "0") converted = rendered[i]..converted
 end
 while (#converted < 8) do
  converted ..= "0"
 end
 return (no_symbol and "" or c_btc_symbol_render)..converted
end

-- https://www.lexaloffle.com/bbs/?pid=22809#p
function float_to_money_str(v, no_symbol)
 ...
 repeat
  ...
  i+=1
 until v==0
 return render_money(s, no_symbol)
end
...
intermission_facts = split("more bitcoin is mined every_10 minutes than was mined_in the entire middle ages?~bitcoin has finite supply_of coins but infinite supply_of memes?~the bitcoin whitepaper is_only 9 pages long and is_mostly cookie recipes?~the term 'stacking sats'_means putting sats on top of_other sats?~satoshi nakamoto's identity_is a secret, but his/her_uncle is bronson pinchot?~conditions in bitcoin mines_improved dramatically after_the 1873 westville disaster?~there can only be 21 million_bitcoin because computers_can't handle bigger numbers?~physical bitcoins do not_actually exist, but they_sure look pretty fancy?~satoshi's list of possible_names for bitcoin included_'hypercash'?~satoshi's list of possible_names for bitcoin included_'incredicoin'?~satoshi's list of possible_names for bitcoin included_'awesomoney'?~satoshi's list of possible_names for bitcoin included_'megamoola'?~satoshi's list of possible_names for bitcoin included_'vbucks'?~satoshi's list of possible_names for bitcoin included_'cybershekels'?~satoshi's list of possible_names for bitcoin included_'ledgerloot'?~satoshi's list of possible_names for bitcoin included_'orangebacks'?~satoshi's list of possible_names for bitcoin included_'bitbenjamins'?~satoshi's list of possible_names for bitcoin included_'digidoubloons'?~satoshi's list of possible_names for bitcoin included_'e-scrilla'?~satoshi's list of possible_names for bitcoin included_'digismackers'?", "~")
